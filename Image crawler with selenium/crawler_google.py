...
searchterm = 'scrub typhus eschar'
...
#주의 :: 이 부분은  꼭 수정하시길 바랍니다. 개인화된 값은 Chrome 브라우저를 열고 chrome://version을 입력하면 온갖 정보들이 나오는데, 이중에서 사용자 에이전트 값을 찾아서 변경하세요.
...
for x in browser.find_elements_by_xpath('//div[contains(@class,"rg_meta")]'):
    ...
    # 구글 이미지를 읽고 저장한다.
    try:
        req = urllib.request.Request(img, headers=header)  # urllib.request가 python3부터는 module이 되었기때문에, 이 기능을 수행하는 class Request를 호출합니다.
